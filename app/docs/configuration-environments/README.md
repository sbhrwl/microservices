...
| Default values             | All optional properties have defaults; configuration values are never null               |
...
| Thread pool configuration | Netty boss thread pool handles connection acceptance; worker pool handles request processing      |
...
| Authentication     | `authMechanism` (default: `"SCRAM-SHA-256"`), credentials loaded from `mongodb-secret.conf`              |
| Connection options | Includes auth mechanism, heartbeat, write concern, retry writes, compression                             |
...
| Expected audience   | `expectedAudience` (default: `"account"`), used for JWT `aud` claim validation                                         |
| Expected issued for | `expectedIssuedFor` (CSV list), used for JWT `azp` claim validation, overridden by `KEYCLOAK_ISSUED_FOR`               |
...
| Aspect            | Details                                                                   |
| ----------------- | ------------------------------------------------------------------------- |
| Multi-stage build | Maven build stage compiles and packages the application                   |
