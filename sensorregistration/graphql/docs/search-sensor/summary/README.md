...
* Define proto files
  * Create `.proto` file for gRPC:
    * `GetSensorRequest`
    * `SensorResponse`
    * `SensorService` with `getSensor` rpc
* Generate grpc code
...
* Build application service
  * Implement `getSensor()` logic:
    * Fetch sensor by id
    * Throw `SensorNotFoundException` if not found
* Implement grpc server (detailed steps)
  * Accept grpc request
    * Read `GetSensorRequest` fields
  * Call application service
    * `applicationService.getSensor(sensorId)`
  * Build grpc response
...
* Implement graphql query service
  * Create method:
    * `getSensor(sensorId)`
    * Uses apollo `query()`
* Call query from component
  * On submit:
    * Call `getSensor(...)`
    * Show sensor details on success
    * Show “not found” when result is null
